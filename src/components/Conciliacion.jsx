...
const CONCEPTO_COLORS = {
  ...
  uniformes_wa:   'bg-purple-500/15 text-purple-300 border-purple-500/20',
  torneo:         'bg-orange-500/15 text-orange-300 border-orange-500/20',
  torneo_wa:      'bg-orange-500/15 text-orange-300 border-orange-500/20',
  otro:           'bg-gray-500/15 text-gray-300 border-gray-500/20',
};

const CONCEPTO_WA_LABELS = {
  mensualidad_wa: 'mensualidad WA',
  torneo_wa:      'torneo WA',
  uniformes_wa:   'uniformes WA',
};
    ...
    <tr className={`border-b border-[var(--cc20)] hover:bg-white/[0.02] transition-colors ${pago.alerta_fraude ? 'bg-red-500/[0.04]' : ''}`}>
      ...
      <td className="px-4 py-3">
        <span className={`inline-flex px-2 py-0.5 rounded-lg border text-xs font-medium ${CONCEPTO_COLORS[pago.concepto] || CONCEPTO_COLORS.otro}`}>
          {CONCEPTO_WA_LABELS[pago.concepto] || pago.concepto}
        </span>
      </td>
