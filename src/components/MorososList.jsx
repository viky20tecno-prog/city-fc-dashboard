        ...
        {morosos.map((m, i) => (
          <div key={m.cedula || i} className="flex items-center justify-between p-3 rounded-xl bg-[rgba(255,94,94,0.06)] border border-[#30363D]">
            <div>
              ...
              <p className="text-xs text-[#8B949E]">CC {m.cedula} · {m.dias_mora} días de mora</p>
            </div>
            <div className="text-right">
              <p className="font-semibold text-[#FF5E5E] text-sm">{formatCOP(m.monto_pendiente)}</p>
              <a href={`tel:${m.celular}`} className="text-xs text-[#8B949E] hover:text-[#4A9EFF] inline-flex items-center gap-1">
