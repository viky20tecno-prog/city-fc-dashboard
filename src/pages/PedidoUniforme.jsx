...
export default function PedidoUniforme() {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-app)]">
      {/* Header */}
      <header className="bg-[var(--bg-card)] border-b border-[var(--cc20)]">
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
        <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--cc20)] p-8">
          <form onSubmit={handleSubmit} className="space-y-6">
            
            {/* Mensaje */}
            {message.text && (
              <div className={`flex items-start gap-3 p-4 rounded-lg ${message.type === 'error' ? 'bg-[rgba(255,94,94,0.12)] border border-[#FF5E5E]/20' : 'bg-[var(--cc12)] border border-[var(--cc)]/20'}`}>
                {message.type === 'error' ? (
                  <AlertCircle className="w-5 h-5 text-[#FF5E5E] shrink-0 mt-0.5" />
                ) : (
                  <CheckCircle className="w-5 h-5 text-[var(--cc)] shrink-0 mt-0.5" />
                )}
                <span className={`text-sm ${message.type === 'error' ? 'text-[#FF5E5E]' : 'text-[var(--cc)]'}`}>
                  {message.text}
                </span>
              </div>
            )}
            ...
            <div>
              ...
              {jugadorSeleccionado && (
                <p className="text-xs text-[var(--cc)] mt-2">
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 px-6 py-3 bg-[var(--cc)] text-[#060C18] rounded-lg font-semibold hover:bg-[var(--cc)]/80 transition disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? (
                <>
